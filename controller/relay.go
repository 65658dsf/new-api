...
func executeTaskSubmissionWith(
	...
	baseQuota := (*float64)(nil)
	if groupRatio := relayInfo.PriceData.GroupRatioInfo.GroupRatio; groupRatio > 0 {
		baseQuotaValue := float64(result.Quota) / groupRatio
		baseQuota = &baseQuotaValue
	}
	service.RecordChannelFinancialConsumeWithReference(c, relayInfo, relayInfo.OriginModelName, result.Quota, baseQuota, task.TaskID)
